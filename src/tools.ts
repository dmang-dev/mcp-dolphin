...
const TOOLS: Tool[] = [
...

  {
    ...
    description:
      "PURPOSE: Capture the current Dolphin video frame as a PNG image. " +
      "USAGE: Use to SEE what's on screen — confirm game state, read a menu/HUD, or document a moment — to complement memory reads with vision. Returns the most recently DRAWN frame: near-live while the game runs, or the frozen frame if Dolphin is paused via its GUI. " +
      "BEHAVIOR: No side effects — pure read of the last rendered framebuffer. The bridge captures frames via Felk's event.on_framedrawn (RGB) and mcp-dolphin encodes the PNG. Returns an error if this Dolphin build doesn't expose on_framedrawn (older Felk builds), or if no frame has been drawn yet (let the game render a frame, then retry). Image dimensions follow Dolphin's output framebuffer, which scales with the internal-resolution setting. " +
      "RETURNS: A one-line summary (WIDTH×HEIGHT) plus the screenshot as an inline PNG image.",
    inputSchema: { type: "object", properties: {} },
  },
...
// The bridge hands us a raw RGB framebuffer (Felk's on_framedrawn → width*height*3
// bytes); MCP image content needs a real image format. We assemble a truecolor
// PNG by hand: IHDR + a single zlib-deflated IDAT (filter byte 0 per scanline) +
// IEND. node:zlib does the compression; CRC32 is computed locally.
...
function rgbToPng(width: number, height: number, rgb: Buffer): Buffer {
  ...
  if (rgb.length < needed) {
    throw new Error(`framebuffer too small: got ${rgb.length} bytes, expected ${needed} for ${width}x${height} RGB`);
  }
  // Prefix each scanline with filter-type byte 0 (None).
...
export function registerTools(server: Server, dol: DolphinClient): void {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    switch (name) {
      ...
      case "dolphin_screenshot": {
        const r = await dol.call<{ width: number; height: number; format: string; rgb_base64: string }>("gui.screenshot");
        const rgb = Buffer.from(r.rgb_base64, "base64");
        const png = rgbToPng(r.width, r.height, rgb);
        return {
          content: [
            { type: "text" as const, text: `Screenshot ${r.width}×${r.height} (PNG, ${png.length} bytes).` },
